...
#ifdef CONFIG_BACKLIGHT_MAX8957
int primoc_flashlight_workaround = 1;
#endif
enum {
	GATE_ON = 1 << 0,
};

enum led_brightness primoc_brightness_value = DEFAULT_BRIGHTNESS;

static struct workqueue_struct *primoc_cabc_wq;
static struct delayed_work primoc_cabc_work;

extern unsigned long msm_fb_base;

static int
primoc_shrink_pwm(int brightness, int user_def,
		int user_min, int user_max, int panel_def,
		int panel_min, int panel_max)
{
/*
 *  This workaround is for camera flashlight
 *  to turn off backlight power because primo#c's
 *  PMIC can't turn off the backlight while change
 *  the brightness to zero.
 *  You need turn off the backlight power.
 */

#ifdef CONFIG_BACKLIGHT_MAX8957
	if (brightness == 0 && primoc_flashlight_workaround == 1) {
		if (board_mfg_mode() != 4)
			max8957_set_bl_on_off(MAX8957_WLED_1, MAX8957_WLED_DIS);
		primoc_flashlight_workaround = 0;
	} else if (brightness > 0 && primoc_flashlight_workaround == 0){
		max8957_set_bl_on_off(MAX8957_WLED_1, MAX8957_WLED_EN);
		primoc_flashlight_workaround = 1;
	}
#endif
...
static void primoc_cabc_open(struct work_struct *w)
{
...
static void
primoc_set_brightness(struct led_classdev *led_cdev,
				enum led_brightness val)
{
	...
	shrink_br = primoc_shrink_pwm(val, PWM_USER_DEF,
	...
	if (cabc.last_shrink_br == 0 && shrink_br)
		...
		queue_delayed_work(primoc_cabc_wq, &primoc_cabc_work, 100);

	/* Update the last brightness */
	cabc.last_shrink_br = shrink_br;
	primoc_brightness_value = val;
...
primoc_get_brightness(struct led_classdev *led_cdev)
{

	return primoc_brightness_value;
}

static void
primoc_backlight_switch(int on)
{
	enum led_brightness val;

	if (on) {
		printk(KERN_DEBUG "turn on backlight\n");
// TESTING CRT		max8957_set_bl_on_off(MAX8957_WLED_1, MAX8957_WLED_EN);
...
// TESTING CRT		max8957_set_bl_on_off(MAX8957_WLED_1, MAX8957_WLED_DIS);
		primoc_set_brightness(&cabc.lcd_backlight, 0);
	}
}

static int
primoc_ce_switch(int on)
{
	struct msm_mddi_client_data *client = cabc.client_data;
...
static int
primoc_backlight_probe(struct platform_device *pdev)
{
	int err = -EIO;
	...
	cabc.lcd_backlight.brightness_set = primoc_set_brightness;
	cabc.lcd_backlight.brightness_get = primoc_get_brightness;
	...
	primoc_cabc_wq = alloc_ordered_workqueue("cabc_wq", 0);
	INIT_DELAYED_WORK(&(primoc_cabc_work), primoc_cabc_open);
...
struct nov_regs primoc_sony_init_seq[] = {
	{0x1100, 0x00},
	{REG_WAIT, 120},
	{0xF000, 0x55},
	{0xF001, 0xaa},
...
static int
primoc_mddi_init(struct msm_mddi_bridge_platform_data *bridge_data,
		     struct msm_mddi_client_data *client_data)
{
	...
	case PANEL_ID_PRIMO_SONY:
		init_seq = primoc_sony_init_seq;
		array_size = ARRAY_SIZE(primoc_sony_init_seq);
		PR_DISP_DEBUG("%s: panel_type_SONY",__func__);
		break;
	case PANEL_ID_PRIMO_LG:
	default:
		init_seq = pro_lgd_init_seq;
		array_size = ARRAY_SIZE(pro_lgd_init_seq);
		PR_DISP_DEBUG("%s: panel_type_LG",__func__);
		break;
	}
...
static int
primoc_mddi_uninit(struct msm_mddi_bridge_platform_data *bridge_data,
			struct msm_mddi_client_data *client_data)
{
	B(KERN_DEBUG "%s(%d)\n", __func__, __LINE__);
	return 0;
...
static int
primoc_panel_blank(struct msm_mddi_bridge_platform_data *bridge_data,
			struct msm_mddi_client_data *client_data)
{
	B(KERN_DEBUG "%s(%d)\n", __func__, __LINE__);
...
static int
primoc_panel_unblank(struct msm_mddi_bridge_platform_data *bridge_data,
			struct msm_mddi_client_data *client_data)
{
	B(KERN_DEBUG "%s\n", __func__);
	client_data->auto_hibernate(client_data, 0);
	...
	primoc_backlight_switch(LED_FULL);
	client_data->auto_hibernate(client_data, 1);
	return 0;
}

static struct msm_mddi_bridge_platform_data novatec_client_data = {
	.init = primoc_mddi_init,
	.uninit = primoc_mddi_uninit,
	.blank = primoc_panel_blank,
	.unblank = primoc_panel_unblank,
	.fb_data = {
		.xres = 480,
		.yres = 800,
		.width = 52,
...
mddi_power(struct msm_mddi_client_data *client_data, int on)
{
	...
	if (on) {
		...
		/*Set LCD_ID0(GPIO_129) to INPUT NO-PULL*/
		config = PCOM_GPIO_CFG(PRIMOC_LCD_ID0, 0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA);
		rc = msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
		/*Set LCD_ID1(GPIO_128) to OUTPUT HIGH (for selecting MDDI interface)*/
		config = PCOM_GPIO_CFG(PRIMOC_LCD_ID1, 0, GPIO_OUTPUT, GPIO_PULL_UP, GPIO_2MA);
		rc = msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
		...
		max8957_set_bl_on_off(MAX8957_WLED_1, MAX8957_WLED_DIS);
#endif
		/*Set LCD_RST# to LOW in order to hold LCM at reset state*/
		gpio_set_value(PRIMOC_LCD_RSTz, 0);
		/*Set LCD_ID0(GPIO_129) to INPUT PULL-DOWN*/
		config = PCOM_GPIO_CFG(PRIMOC_LCD_ID0, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA);
		rc = msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
		/*Set LCD_ID0(GPIO_128) to INPUT PULL-DOWN*/
		config = PCOM_GPIO_CFG(PRIMOC_LCD_ID1, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA);
		rc = msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
		hr_msleep(20);
...
static struct platform_driver primoc_backlight_driver = {
	.probe = primoc_backlight_probe,
	.driver = {
		.name = "nov_cabc",
		.owner = THIS_MODULE,
	},
...
int __init primoc_init_panel(void)
{
...
#if 1 	
	/* roll back to 384MHz due to mddi init failed*/
	mddi_pdata.clk_rate = 384000000;
#else
	/* Change PMDH clock rate from 384MHz to 400MHz for EMC issue */
	mddi_pdata.clk_rate = 400000000;
#endif
	...
	rc = platform_driver_register(&primoc_backlight_driver);
	if (rc)
		return rc;
